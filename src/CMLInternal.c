...
//  const float* whitepointYxy = cmlGetWhitepointYxy(cm->observer);
...
//  cmlInternalXYZtoCIELAB(out, in, count, cmlGetWhitepointInverseXYZ(cmlGetLabSpaceWhitepoint(cm->labspace)), cmlGetResponseCurveForwardFunc(cm->labspace.responseL));
//}
//
//CMLHIDDEN void CMLInternalXYZtoLabCIELAB_SB(const CMLColorMachine* cm, float* buf, CMLuint32 count){
//  cmlInternalXYZtoCIELAB_SB(buf, count, cmlGetWhitepointInverseXYZ(cmlGetLabSpaceWhitepoint(cm->labspace)), cmlGetResponseCurveForwardFunc(cm->labspace.responseL));
//}
//
//
...
//  const float* inverseWhitepointXYZ = cmlGetWhitepointInverseXYZ(cm->observer);
//  while(count){
//
...
//    
//    float fy = cmlInternalEval(cmlGetResponseCurveForwardFunc(cm->labspace.responseL), yr);
//    out[0] = 100.f * fy;
//
//    if(fy == 0.f){
//      out[1] = 0.f;
...
//  cmlInternalCIELABtoXYZ(out, in, count, cmlGetWhitepointXYZ(cmlGetLabSpaceWhitepoint(cm->labspace)), cmlGetResponseCurveBackwardFunc(cm->labspace.responseL));
//}


//CMLHIDDEN void CMLInternalLabtoXYZCIELAB_SB (const CMLColorMachine* cm, float* buf, CMLuint32 count){
//  cmlInternalCIELABtoXYZ_SB(buf, count, cmlGetWhitepointXYZ(cmlGetLabSpaceWhitepoint(cm->labspace)), cmlGetResponseCurveBackwardFunc(cm->labspace.responseL));
//}
...
//  const float* whitepointXYZ = cmlGetWhitepointXYZ(cmlGetLabSpaceWhitepoint(cm->labspace));
//  while(count){
//
//    float xr, yr, zr;
//    
//    float fy = in[0] * .01f;
//    yr = cmlInternalEval(cmlGetResponseCurveBackwardFunc(cm->labspace.responseL), fy);
//
...
//  const float* whitepointXYZ = cmlGetWhitepointXYZ(cm->observer);
//  while(count){
//
//    float xr, yr, zr;
//    

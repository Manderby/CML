...
CML_HIDDEN CMLConverterChainElement* cml_CreateConverterChainElement(CMLColorspace* dstspace){
  CMLConverterChainElement* newelement = (CMLConverterChainElement*)naAllocate(sizeof(CMLConverterChainElement));
...
CML_API CMLConverter* cmlCreateConverter(
  ...
  if(!mobEqual(cml_Key(CML_COLORSPACE_OBJECT), srcspace))
    cmlError("cmlCreateConverter", "Given MOB is not a Colorspace\n");
  #endif
  CMLConverter* converter = (CMLConverter*)naAllocate(sizeof(CMLConverter));
  converter->cost = 0.f;
  naCreateList(&(converter->elements));

  naAddListElementFirst(&(converter->elements), cml_CreateConverterChainElement(srcspace));
  naAddListElementLast(&(converter->elements), cml_CreateConverterChainElement(dstspace));
...
CML_API void cmlDestroyConverter(CMLConverter* converter){
  ...
  naClearList(&(converter->elements), (NADestructor)cml_DestroyConverterChainElement);
}
...
CML_API void cmlConvert(CMLConverter* converter, CMLInputOutput out, CMLInputOutput in, CMLInt count){
  ...
  if(!last->converterUnit){
    ...
    for(CMLInt i=1; i<chaincount - 1; i++){
      ...
      cur->dstbuf = naAllocate(colorspacesize * count);
    }
...
CML_API void cml_Debug_FinalConverter(CMLConverter* converter){
  ...
  printf("%s", naGetStringConstUTF8Pointer(converterUnitname));
  naNextListElement(&(converter->elements));
  curelement = naGetListMutableContent(&(converter->elements));
  while(curelement){
    ...
    printf(" ->(%f) %s", cost, naGetStringConstUTF8Pointer(converterUnitname));
    naNextListElement(&(converter->elements));
    curelement = naGetListMutableContent(&(converter->elements));
  }

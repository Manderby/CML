...
CMLAPI void cmlDestroyObserver(void* observer);
...
CMLAPI void cmlCreateSpectralDistributionFunctions(CMLFunction* functions[3], CMLObserverType type){
  switch(type){
  ...
  default:
    ...
    #ifndef NDEBUG
      cmlError("cmlCreateSpectralDistributionFunctions", "Observer type invalid");
    #endif
    break;
  }
}
...
CMLAPI CMLObserver* cmlCreateObserverWithPreset(CMLObserverType type){
  if((type <= CML_OBSERVER_CUSTOM) || (type >= CML_NUMBER_OF_OBSERVERS)){
    #ifndef NDEBUG
      cmlError("cmlCreateObserverWithPreset", "Observer type invalid");
    #endif
    return CML_NULL;
  }

  CMLObserver* observer = cmlCreateObject(sizeof(CMLObserver), cmlDestroyObserver);
  observer->type = type;
  cmlCreateSpectralDistributionFunctions(observer->specdistfunctions, observer->type);
  return observer;
}



CMLAPI void cmlDestroyObserver(void* obs){
  CMLObserver* observer = (CMLObserver*)obs;
  cmlReleaseObject(observer->specdistfunctions[0]);
  cmlReleaseObject(observer->specdistfunctions[1]);
  cmlReleaseObject(observer->specdistfunctions[2]);
}
...
CMLAPI const CMLFunction* cmlGetObserverSpectralDistributionFunction(const CMLObserver* observer, CMLInt index){
...
CMLAPI void cmlGetSpectralColorXYZ(   const CMLObserver* observer,
                                                 CMLVec3 xyz,
                                                   float lambda){
  cmlSet3(  xyz,
            cmlInternalEval(observer->specdistfunctions[0], lambda),
            cmlInternalEval(observer->specdistfunctions[1], lambda),
            cmlInternalEval(observer->specdistfunctions[2], lambda));
}

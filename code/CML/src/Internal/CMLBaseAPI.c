...
CML_DEF void cmlConvertYuvToUVW(CMLVec3 uvw, const CMLVec3 yuv, const CMLVec3 whitePointYuv){
  cml_OneYuvToUVW(uvw, yuv, whitePointYuv);
}

CML_DEF void cmlConvertUVWToYuv(CMLVec3 yuv, const CMLVec3 uvw, const CMLVec3 whitePointYuv){
  cml_OneYuvToUVW(yuv, uvw, whitePointYuv);
}
...
CML_DEF void cmlConvertXYZToLab(CMLVec3 lab, const CMLVec3 xyz, const CMLVec3 whitePointXYZ){
  CMLFunction* linearToLStarResponse = cmlCreateYToLStarResponse();
  CMLVec3 whitePointXYZInverse = {cmlInverse(whitePointXYZ[0]), cmlInverse(whitePointXYZ[1]), cmlInverse(whitePointXYZ[2])};
  cml_OneXYZToCIELAB(lab, xyz, whitePointXYZInverse, linearToLStarResponse);
  cmlReleaseFunction(linearToLStarResponse);
}
...
CML_DEF void cmlConvertRGBToYCbCr(CMLVec3 ycbcr, const CMLVec3 rgb, const CMLVec3 redPrimaryYxy, const CMLVec3 bluePrimaryYxy, const CMLVec3 whitePointXYZ){
  ...
  cmlSet3(whitePointXYZInverse, cmlInverse(whitePointXYZ[0]), cmlInverse(whitePointXYZ[1]), cmlInverse(whitePointXYZ[2]));
  cml_OneRGBToYCbCr(ycbcr, rgb, redPrimaryYxy, bluePrimaryYxy, whitePointXYZInverse);
}
...
CML_DEF void cmlConvertYCbCrToRGB(CMLVec3 rgb, const CMLVec3 ycbcr, const CMLVec3 redPrimaryYxy, const CMLVec3 bluePrimaryYxy, const CMLVec3 whitePointXYZ){
  CMLVec3 whitePointXYZInverse;
  cmlSet3(whitePointXYZInverse, cmlInverse(whitePointXYZ[0]), cmlInverse(whitePointXYZ[1]), cmlInverse(whitePointXYZ[2]));
  cml_OneYCbCrToRGB(rgb, ycbcr, redPrimaryYxy, bluePrimaryYxy, whitePointXYZInverse);
}
...
CML_DEF void cmlConvertRemissionSpectrumToXYZ(CMLVec3 xyz, const CMLFunction* specRem, const CMLFunction* specIll, const CMLObserver* observer, float radiometricScale, const CMLIntegration* integration){
  cml_OneRemissionSpectrumToXYZ(xyz, specRem, specIll, observer, radiometricScale, integration);
}
...
CML_DEF size_t cmlGetNumChannels(CMLColorType colorType){
  ...
  case CML_COLOR_Lch:   return CML_Lch_CHANNEL_COUNT;   break;
  case CML_COLOR_Luv:   return CML_Luv_CHANNEL_COUNT;   break;
  case CML_COLOR_RGB:   return CML_RGB_CHANNEL_COUNT;   break;
  case CML_COLOR_UVW:   return CML_UVW_CHANNEL_COUNT;   break;
  case CML_COLOR_XYZ:   return CML_XYZ_CHANNEL_COUNT;   break;
  case CML_COLOR_YCbCr: return CML_YCbCr_CHANNEL_COUNT; break;
  case CML_COLOR_Ycd:   return CML_Ycd_CHANNEL_COUNT;   break;
  case CML_COLOR_Yupvp: return CML_Yupvp_CHANNEL_COUNT; break;
  case CML_COLOR_Yuv:   return CML_Yuv_CHANNEL_COUNT;   break;
  case CML_COLOR_Yxy:   return CML_Yxy_CHANNEL_COUNT;   break;
  default:
    #if CML_DEBUG
...
CML_DEF void cmlGetMinBounds(float* buffer, CMLColorType colorType){
  ...
  case CML_COLOR_UVW:   cmlSet3(buffer, CML_UVW_U_MIN,   CML_UVW_V_MIN,    CML_UVW_W_MIN);    break;
...
CML_DEF void cmlGetMaxBounds(float* buffer, CMLColorType colorType){
  ...
  case CML_COLOR_UVW:   cmlSet3(buffer, CML_UVW_U_MAX,   CML_UVW_V_MAX,    CML_UVW_W_MAX);    break;

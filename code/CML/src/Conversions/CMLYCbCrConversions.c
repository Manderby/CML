...
CML_API void cmlYCbCrToYCbCr(const CMLColorMachine* cm, CMLOutput ycbcrd, CMLInput ycbcrs, size_t count){
  CML_UNUSED(cm);
  CML_CONVERTER_INOUT_PARAMETER(ycbcrd, ycbcrs, CML_YCbCr_CHANNEL_COUNT, CML_YCbCr_CHANNEL_COUNT);
  CML_CONVERTER_MEMCPY(CML_YCbCr_CHANNEL_COUNT);
}
...
CML_API void cmlYCbCrToCMYK(const CMLColorMachine* cm, CMLOutput cmyk, CMLInput ycbcr, size_t count){
  CML_CONVERTER_INOUT_PARAMETER(cmyk, ycbcr, CML_CMYK_CHANNEL_COUNT, CML_YCbCr_CHANNEL_COUNT);
...
CML_API void cmlYCbCrToHSL(const CMLColorMachine* cm, CMLOutput hsl, CMLInput ycbcr, size_t count){
  CML_CONVERTER_INOUT_PARAMETER(hsl, ycbcr, CML_HSL_CHANNEL_COUNT, CML_YCbCr_CHANNEL_COUNT);
  CML_CONVERTER_FIRST_STEP(cml_CMYCbCrToRGB);
...
CML_API void cmlYCbCrToHSV(const CMLColorMachine* cm, CMLOutput hsv, CMLInput ycbcr, size_t count){
  CML_CONVERTER_INOUT_PARAMETER(hsv, ycbcr, CML_HSV_CHANNEL_COUNT, CML_YCbCr_CHANNEL_COUNT);
  CML_CONVERTER_FIRST_STEP(cml_CMYCbCrToRGB);
  cml_CMRGBToHSV_SB(cm, out, count, floatAlign);
}

CML_API void cmlYCbCrToLab(const CMLColorMachine* cm, CMLOutput lab, CMLInput ycbcr, size_t count){
  CML_CONVERTER_INOUT_PARAMETER(lab, ycbcr, CML_Lab_CHANNEL_COUNT, CML_YCbCr_CHANNEL_COUNT);
  CML_CONVERTER_FIRST_STEP(cml_CMYCbCrToRGB);
...
CML_API void cmlYCbCrToLch(const CMLColorMachine* cm, CMLOutput lch, CMLInput ycbcr, size_t count){
  CML_CONVERTER_INOUT_PARAMETER(lch, ycbcr, CML_Lch_CHANNEL_COUNT, CML_YCbCr_CHANNEL_COUNT);
  CML_CONVERTER_FIRST_STEP(cml_CMYCbCrToRGB);
  cml_CMRGBToXYZ_SB(cm, out, count, floatAlign);
  (*cm->XYZToLab_SB)(cm, out, count, floatAlign);
...
CML_API void cmlYCbCrToLuv(const CMLColorMachine* cm, CMLOutput luv, CMLInput ycbcr, size_t count){
  CML_CONVERTER_INOUT_PARAMETER(luv, ycbcr, CML_Luv_CHANNEL_COUNT, CML_YCbCr_CHANNEL_COUNT);
  CML_CONVERTER_FIRST_STEP(cml_CMYCbCrToRGB);
  cml_CMRGBToXYZ_SB(cm, out, count, floatAlign);
...
CML_API void cmlYCbCrToRGB(const CMLColorMachine* cm, CMLOutput rgb, CMLInput ycbcr, size_t count){
  CML_CONVERTER_INOUT_PARAMETER(rgb, ycbcr, CML_RGB_CHANNEL_COUNT, CML_YCbCr_CHANNEL_COUNT);
  CML_CONVERTER_FIRST_STEP(cml_CMYCbCrToRGB);
}

CML_API void cmlYCbCrToUVW(const CMLColorMachine* cm, CMLOutput uvw, CMLInput ycbcr, size_t count){
  CML_CONVERTER_INOUT_PARAMETER(uvw, ycbcr, CML_Yuv_CHANNEL_COUNT, CML_YCbCr_CHANNEL_COUNT);
  CML_CONVERTER_FIRST_STEP(cml_CMYCbCrToRGB);
  cml_CMRGBToXYZ_SB(cm, out, count, floatAlign);
  cml_CMXYZToYxy_SB(cm, out, count, floatAlign);
  cml_CMYxyToYupvp_SB(cm, out, count, floatAlign);
...
CML_API void cmlYCbCrToXYZ(const CMLColorMachine* cm, CMLOutput xyz, CMLInput ycbcr, size_t count){
  CML_CONVERTER_INOUT_PARAMETER(xyz, ycbcr, CML_XYZ_CHANNEL_COUNT, CML_YCbCr_CHANNEL_COUNT);
  CML_CONVERTER_FIRST_STEP(cml_CMYCbCrToRGB);
  cml_CMRGBToXYZ_SB(cm, out, count, floatAlign);
}

CML_API void cmlYCbCrToYcd(const CMLColorMachine* cm, CMLOutput ycd, CMLInput ycbcr, size_t count){
  CML_CONVERTER_INOUT_PARAMETER(ycd, ycbcr, CML_Ycd_CHANNEL_COUNT, CML_YCbCr_CHANNEL_COUNT);
  CML_CONVERTER_FIRST_STEP(cml_CMYCbCrToRGB);
  cml_CMRGBToXYZ_SB(cm, out, count, floatAlign);
  cml_CMXYZToYxy_SB(cm, out, count, floatAlign);
  cml_CMYxyToYupvp_SB(cm, out, count, floatAlign);
...
CML_API void cmlYCbCrToYupvp(const CMLColorMachine* cm, CMLOutput yupvp, CMLInput ycbcr, size_t count){
  CML_CONVERTER_INOUT_PARAMETER(yupvp, ycbcr, CML_Yupvp_CHANNEL_COUNT, CML_YCbCr_CHANNEL_COUNT);
  CML_CONVERTER_FIRST_STEP(cml_CMYCbCrToRGB);
  cml_CMRGBToXYZ_SB(cm, out, count, floatAlign);
  cml_CMXYZToYxy_SB(cm, out, count, floatAlign);
  cml_CMYxyToYupvp_SB(cm, out, count, floatAlign);
...
CML_API void cmlYCbCrToYuv(const CMLColorMachine* cm, CMLOutput yuv, CMLInput ycbcr, size_t count){
  CML_CONVERTER_INOUT_PARAMETER(yuv, ycbcr, CML_Yuv_CHANNEL_COUNT, CML_YCbCr_CHANNEL_COUNT);
  CML_CONVERTER_FIRST_STEP(cml_CMYCbCrToRGB);
  cml_CMRGBToXYZ_SB(cm, out, count, floatAlign);
  cml_CMXYZToYxy_SB(cm, out, count, floatAlign);
  cml_CMYxyToYupvp_SB(cm, out, count, floatAlign);
...
CML_API void cmlYCbCrToYxy(const CMLColorMachine* cm, CMLOutput yxy, CMLInput ycbcr, size_t count){
  CML_CONVERTER_INOUT_PARAMETER(yxy, ycbcr, CML_Yxy_CHANNEL_COUNT, CML_YCbCr_CHANNEL_COUNT);
  CML_CONVERTER_FIRST_STEP(cml_CMYCbCrToRGB);
  cml_CMRGBToXYZ_SB(cm, out, count, floatAlign);
  cml_CMXYZToYxy_SB(cm, out, count, floatAlign);
}
